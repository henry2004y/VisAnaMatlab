% Script for capturing FTEs and doing statistics in G28 simulations.
% Procedures:
...
for ipict = 1:npict
   ...
   subplot_tight(3,3,1);
   %scatter(y3bc,z3bc,[],bxv,'filled'); colorbar;   
   ...
   subplot_tight(3,3,2);
   %scatter(y3bc,z3bc,[],byv,'filled'); colorbar;
   ...
   for k = 1 : FTEcountJ
      % switch to grid axis
      thisBoundary = (boundaries{k}-1)./30 + [min(min(yq)) min(min(zq))];
